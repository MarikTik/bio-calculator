...
def shorten_codon_name(codon_name: str) -> str:
    """Extracts the shortened codon name 

    Args:
        codon_name (str): A codon name containing descriptions in () parantheses

    Returns:
        str: The part of a codon name without descriptions
    """
    match = re.match(r".*?\((.*?)\)", codon_name)
    return match.group(1) if match else codon_name
